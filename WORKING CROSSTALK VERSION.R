...
```{r tab_1_sidebar}
filter_checkbox("team_filter", 
                label = "Team:", 
                sharedData = shared_events, 
                group = ~events$team$name,
                inline = T)

filter_checkbox("half_filter", 
...
# MAIN PLOT OUTPUT
# The SharedData filtering happens automatically - no server code needed
p <- 
  shared_pass %>%
  ...
  geom_point(aes(x = loc_L, y = loc_W, color = pass_outcome_name, shape = pass_outcome_name),
             size = 0.8) +
  facet_grid(rows=vars(team$name), cols=vars(period)) +
  scale_color_manual(values = c("green", "red", "gold", "purple", "pink")) +
  scale_shape_manual(values = c(1, 4, 4, 4, 2)) +
  theme_minimal()

# Create converted plot
p_plotly <- ggplotly(p, height = 600)

# Express plot with fixed legend names
fix_ggplotly_legends(p_plotly)
```
